...
public class MyBorrowTab {
    ...
    public Tab build() {
        ...
        Runnable loadRecords = () -> {
            ...
            ctx.getExecutorService().submit(() -> {
                try {
                    ...
                    Result<PageResponse<BorrowRecordDTO>> result = ctx.getBookServiceClient().getBorrowRecordsByUser(userId, currentPage[0], pageSize);
                    Platform.runLater(() -> {
        ...
        returnBtn.setOnAction(e -> {
            ...
            ctx.getExecutorService().submit(() -> {
                try {
                    Result<BorrowRecordDTO> result = ctx.getBookServiceClient().returnBook(selected.getId());
                    Platform.runLater(() -> {
                        returnBtn.setDisable(false);
                        if (result.getCode() == 200) { statusLabel.setTextFill(Color.GREEN); statusLabel.setText("归还成功"); loadRecords.run(); }
                        else { statusLabel.setTextFill(Color.RED); statusLabel.setText("归还失败：" + result.getMessage()); }
                    });
                } catch (Exception ex) {
                    Platform.runLater(() -> { returnBtn.setDisable(false); statusLabel.setTextFill(Color.RED); statusLabel.setText("归还失败：" + ex.getMessage()); });
                }
            });
        });
        ...
        renewBtn.setOnAction(e -> {
            ...
            ctx.getExecutorService().submit(() -> {
                try {
                    Result<BorrowRecordDTO> result = ctx.getBookServiceClient().renewBook(selected.getId());
                    Platform.runLater(() -> {
                        renewBtn.setDisable(false);
                        if (result.getCode() == 200) { statusLabel.setTextFill(Color.GREEN); statusLabel.setText("续借成功"); loadRecords.run(); }
                        else { statusLabel.setTextFill(Color.RED); statusLabel.setText("续借失败：" + result.getMessage()); }
                    });
                } catch (Exception ex) {
                    Platform.runLater(() -> { renewBtn.setDisable(false); statusLabel.setTextFill(Color.RED); statusLabel.setText("续借失败：" + ex.getMessage()); });
                }
            });
        });
