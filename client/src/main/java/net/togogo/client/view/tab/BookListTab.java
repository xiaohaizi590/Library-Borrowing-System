        ...
        borrowBtn.setOnAction(e -> {
            ...
            dialog.showAndWait().ifPresent(daysStr -> {
                try {
                    ...
                    ctx.getExecutorService().submit(() -> {
                        try {
                            Result<BorrowRecordDTO> borrowResult = ctx.getBookServiceClient().borrowBook(req);
                            Platform.runLater(() -> {
                                borrowBtn.setDisable(false);
                                if (borrowResult.getCode() == 200) {
                                    statusLabel.setTextFill(Color.GREEN);
                                    statusLabel.setText("借阅成功！");
                                    loadBooks.run();
                                } else {
                                    statusLabel.setTextFill(Color.RED);
                                    statusLabel.setText("借阅失败：" + borrowResult.getMessage());
                                }
                            });
                        } catch (Exception ex) {
                            Platform.runLater(() -> {
