...
public class BookManageTab {
    ...
    public Tab build() {
        ...
        Runnable loadData = () -> {
            ...
            ctx.getExecutorService().submit(() -> {
                try {
                    Result<PageResponse<BookDTO>> result;
                    String keyword = searchField.getText().trim();
                    String mode = searchType.getValue();
                    if (keyword.isEmpty()) {
                        result = ctx.getBookServiceClient().getAllBooks(currentPage[0], pageSize);
                    } else {
                        result = switch (mode) {
                            case "按作者" -> ctx.getBookServiceClient().searchByAuthor(keyword, currentPage[0], pageSize);
                            case "按分类" -> ctx.getBookServiceClient().searchByCategory(keyword, currentPage[0], pageSize);
                            default -> ctx.getBookServiceClient().searchByTitle(keyword, currentPage[0], pageSize);
                        };
                    }
                    Platform.runLater(() -> {
                        if (result.getCode() == 200 && result.getData() != null) {
                            PageResponse<BookDTO> page = result.getData();
        ...
        deleteBtn.setOnAction(e -> {
            ...
            confirm.showAndWait().ifPresent(res -> {
                if (res == ButtonType.OK) {
                    ...
                    ctx.getExecutorService().submit(() -> {
                        try {
                            Result<Void> result = ctx.getBookServiceClient().deleteBook(selected.getId());
                            Platform.runLater(() -> {
                                if (result.getCode() == 200) { statusLabel.setTextFill(Color.GREEN); statusLabel.setText("删除成功"); loadData.run(); }
                                else { statusLabel.setTextFill(Color.RED); statusLabel.setText("删除失败：" + result.getMessage()); }
                            });
                        } catch (Exception ex) {
                            Platform.runLater(() -> { statusLabel.setTextFill(Color.RED); statusLabel.setText("删除失败：" + ex.getMessage()); });
                        }
                    });
                }
            });
    ...
    private void showBookDialog(BookDTO existing, Runnable onSave, Label statusLabel) {
        ...
        dialog.showAndWait().ifPresent(req -> {
            ...
            ctx.getExecutorService().submit(() -> {
                try {
                    Result<BookDTO> result = existing == null ? ctx.getBookServiceClient().createBook(req) : ctx.getBookServiceClient().updateBook(existing.getId(), req);
                    Platform.runLater(() -> {
                        if (result.getCode() == 200) { statusLabel.setTextFill(Color.GREEN); statusLabel.setText(existing == null ? "添加成功" : "更新成功"); onSave.run(); }
                        else { statusLabel.setTextFill(Color.RED); statusLabel.setText("保存失败：" + result.getMessage()); }
                    });
                } catch (Exception ex) {
                    Platform.runLater(() -> { statusLabel.setTextFill(Color.RED); statusLabel.setText("保存失败：" + ex.getMessage()); });
                }
            });
        });
    }
