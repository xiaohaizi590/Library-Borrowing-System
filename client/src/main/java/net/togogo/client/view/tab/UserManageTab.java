    ...
    public Tab build() {
        ...
        Runnable loadData = () -> {
            ...
            ctx.getExecutorService().submit(() -> {
                try {
                    Result<PageResponse<UserDTO>> result = ctx.getUserServiceClient().getAllUsers(currentPage[0], pageSize);
                    Platform.runLater(() -> {
                        if (result.getCode() == 200 && result.getData() != null) {
                            PageResponse<UserDTO> page = result.getData();
        ...
        deleteUserBtn.setOnAction(e -> {
            ...
            confirm.showAndWait().ifPresent(res -> {
                if (res == ButtonType.OK) {
                    ...
                    ctx.getExecutorService().submit(() -> {
                        try {
                            Result<Void> result = ctx.getUserServiceClient().deleteUser(selected.getId());
                            Platform.runLater(() -> {
